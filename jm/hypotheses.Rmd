...
Even with our data free of all non-English entries, there were still a number of entries within the data concerning Wikipedia pages we had no interest in.  For example, Wikipedia images do not link to other pages, so we decided to remove these entries like these from our raw data.  Through our mapreduce_step1.py file, we remove all pagenames that cannot have inlinks associated with them.  Removing these entries helped to decrease our raw English-only data by roughly 25%.  

Additionally, some of the pagenames in the raw data contain percent encoding, meaning we had to remove this percent encoding in the "Step 1".  Finally, since a pagename could theoretically have an entry for every hour in the dataset, we decided to include the datetime in each line of the data, in order to make further processing of the data easier. 
...
The linear regression that was eventually conducted 
